...
import imutils
from Violation_Detection.vehicle import vehicle
...
if tf.__version__ < '1.4.0':
    raise ImportError('Upgrade Tensorflow')
# This is needed to display the images.


sys.path.append("..")

from Violation_Detection.utils import label_map_util

from Violation_Detection.utils import visualization_utils as vis_util
...
PATH_TO_CKPT = os.path.join('Violation_Detection', MODEL_NAME, 'output_inference_graph.pb')

# List of the strings that is used to add correct label for each box.
PATH_TO_LABELS = os.path.join('Violation_Detection', 'Cars', 'car_label_map.pbtxt')
...
secret_key = 'sk_75fc1837b840d016713bde6d'
country = 'us'
recognize_vehicle = 0
state = ''
return_image = 0
...
def getLicensePlateNumber(filer):
    try:
        js = api.recognize_file(filer, secret_key, country, recognize_vehicle=recognize_vehicle, state=state,
                                return_image=return_image, topn=topn, prewarp=prewarp)
        ...
        no = js['results'][0]['plate']
        print(no)

        add = dbconnector.add_licence_plate(no)
        if add == 1:
            print("Plate Entered Sucessfully")

    # print(rimg)
    except ApiException as e:
        plate = "Not Available"
        add = dbconnector.add_licence_plate(plate)
        if add == 1:
            print("Plate Entered Sucessfully in Exception")
        print("Exception: \n", e)
...
#                     color = (255, 0, 0)
#                     thickness = 2
#                     # label= cv.detect_common_objects(image_np)
#
#                     bimg = cv2.putText(bimg, "Signal Violation", org, font, fontscale, color,
#                                        thickness, cv2.LINE_AA)
#                     cv2.imshow('Violator', bimg)
#                     name = 'Violation_Detection/Violation_images/RedLight' + str(time.time()) + '.jpg'
#                     cv2.imwrite(name, bimg)
#                     v_type = "signal"
#                     dbconnector.insert_violation(bimg, v_type)
#
        ...
        if v.speedChecked == False and len(v.points) >= 2:
            ...
            if y2 < yl1 and y2 > yl3 and v.entered == False:
                ...
                if speed > 60:
                    ...
                    bimg = cv2.putText(bimg, "Overspeeding Violation", org, font, fontscale, color,
                                       thickness, cv2.LINE_AA)
                    bimg = cv2.putText(bimg, "Speed:" + str(speed)[:5] + "Km/hr", org_2, font, fontscale,
                                       color_2, thickness, cv2.LINE_AA)
                    cv2.imshow('Violator', bimg)
                    name = 'Violation_Detection/Violation_images/Overspeed' + str(time.time()) + '.jpg'
                    cv2.imwrite(name, bimg)
                    ...
                    store = dbconnector.insert_violation(name, v_type)
...
s_filename = filedialog.askopenfilename(initialdir="/", title="Select A File", filetype=
(("Video files", ".*"), ("all files", ".*")))

cap = cv2.VideoCapture(s_filename)  # 0 is very first webcam attach

# MsgBox = tk.messagebox.askquestion('Traffic Signal?', 'Is Traffic Signal Red?', icon='warning')
# if MsgBox == 'yes':
#     light = 1
# else:
#     light = 0
...
filename = "output.avi"
codec = cv2.VideoWriter_fourcc('m', 'p', '4',
                               'v')  # VideoWriter_fourcc('m','p','4','v')#fourcc means four character code
framerate = 10
resolution = (640, 480)

VideoFileOutput = cv2.VideoWriter(filename, codec, framerate, resolution)
vs = WebcamVideoStream(src=s_filename).start()
ret, imgF = cap.read()
# print(cap.isOpened())
...
start = time.time()
c = 0
sesser = tf.compat.v1.Session(graph=detection_graph)  # tf.Session(graph=detection_graph)
...
window = Toplevel()  # TK() #Makes main window
window.wm_title("Violation Detection")
...
def main(sess=sesser):
    ...
    if True:
        ...
        img = image_np
        # print(boxes," and ", classes, "and ", scores)
        ...
        # cv2.destroyWindow("Vehicle Classification and Traffic Violation Detection System")
        cv2.imshow("Moniter Traffic", cv2.resize(imgF, (1200, 900)))
        if cv2.waitKey(25) & 0xFF == ord('q'):
            cv2.destroyAllWindows()
            cap.release()
    # frame_val = str(cap.get(cv2.CAP_PROP_POS_FRAMES))
    # if frame_val == "2.0":
    # 	cv2.destroyAllWindows()
    # 	print("Inside IF")
...
lbl1 = tk.Label(frame, text='Vehicle Classification and Traffic Violation Detection', font="verdana 12 bold")
lbl1.pack(side='top')
...
def stream():
    ...
    global c
    global tim
    cap = cv2.VideoCapture(s_filename)
    while True:
...
thread.start()'''
with detection_graph.as_default():
    with tf.compat.v1.Session(graph=detection_graph) as sess:  # tf.Session(graph=detection_graph) as sess:
        sesser = sess
        main(sess)  # Display
